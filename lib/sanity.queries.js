...
const prestationOrder = "| order(coalesce(activityDate, publishedAt) desc)";

const createLocalizedQuery = (locale = "fr") => {
  return `*[_type == "prestation" && active == true] ${prestationOrder} {
  _id,
  "title": title.${locale},
...
export const featuredPrestationsQuery = (
  locale = "fr"
) => `*[_type == "prestation" && active == true && featured == true] ${prestationOrder} {
  _id,
  "title": title.${locale},
  "slug": slug.${locale}.current,
  "slugFr": slug.fr.current,
